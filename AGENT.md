...
# Run tests (includes regression tests for all tool types)
...
**Basic functionality:**
- `test_basic_question` - Verifies required output fields and tool call structure
- `test_merge_conflict` - Wiki lookup with `read_file` (regression)
- `test_list_files_in_wiki` - Directory listing with `list_files` (regression)
- `test_path_security` - Verifies path traversal is blocked

**System agent regression:**
- `test_backend_framework` - Source code reading with `read_file` (expects "FastAPI" in answer)
- `test_database_item_count` - API query with `query_api` (expects number in answer)
